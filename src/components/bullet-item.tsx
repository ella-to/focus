...
export const BulletItem = observer(
  ({ bullet, level = 0, onFocus, focusedBulletId, searchQuery = '', onDeleteRequest }: BulletItemProps) => {
    ...
    const getVisibleChildren = ({ bullet, searchQuery, children }: VisibleChildrenFilter): IBullet[] => {
      if (!isAlive(bullet)) return []

      if (searchQuery) {
        return children.filter((child: IBullet) => store.bulletMatchesSearch(child, searchQuery))
      }
    ...
    useEffect(() => {
      ...
      if (contentRef.current && contentRef.current.textContent !== bullet.content) {
        const selection = window.getSelection()
        const isCurrentlyFocused = document.activeElement === contentRef.current
        const cursorPosition = isCurrentlyFocused && selection?.rangeCount ? selection.getRangeAt(0).startOffset : 0
    ...
    useEffect(() => {
      ...
      if (contextRef.current && contextRef.current.textContent !== bullet.context) {
    ...
    return (
      <div
        ...
        style={{
          transform: swipeOffset !== 0 ? `translateX(${Math.max(-100, Math.min(100, swipeOffset))}px)` : undefined,
