...
export const BulletList = observer(() => {
  ...
  const [bulletToDeleteWithChildren, setBulletToDeleteWithChildren] = useState<string | null>(null)
  ...
  const findDeepestLastChild = useCallback((bullet: any): any => {
    ...
    if (!bullet || !bullet.children || bullet.children.length === 0 || bullet.collapsed) {
      return bullet
    }
  ...
  const handleDeleteRequest = useCallback(
    (bulletId: string) => {
      ...
      const previousSiblingId = bulletIndex > 0 ? siblings[bulletIndex - 1].id : null
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      const isInSearchInput = target.tagName === 'INPUT' && target.getAttribute('type') === 'text'
      if (isInSearchInput) {
        return
      }
      ...
      if (isInContext) {
        if (e.key === 'Escape') {
          ...
          if (bulletWrapper) {
            const contentDiv = bulletWrapper.querySelector('.bullet-content') as HTMLDivElement
            if (contentDiv) {
              contentDiv.focus()
      ...
      if (e.key === 'Enter' && !e.shiftKey && !(e.metaKey || e.ctrlKey) && !isInContext) {
        e.preventDefault()
      ...
      if (e.key === 'Enter' && e.shiftKey && !isInContext) {
        e.preventDefault()
        const contextDiv = document.querySelector(`[data-bullet-id="${bullet.id}"] .bullet-context`) as HTMLDivElement
        if (contextDiv) {
          contextDiv.focus()
          setTimeout(() => {
            const range = document.createRange()
      ...
      if (e.key === 'ArrowDown') {
        ...
        if (bullet.children.length > 0 && !bullet.collapsed) {
          ...
          if (parentContext) {
            const parentIndex = parentContext.siblings.findIndex((b: any) => b.id === parent.id)
    ...
    <div className="space-y-1" onKeyDown={handleKeyDown}>
      ...
      {displayBullets.length === 0 && store.searchQuery && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <h3 className="text-lg font-medium text-foreground mb-2">No bullets found</h3>
          <p className="text-sm text-muted-foreground max-w-sm">
            No bullets match your search query "{store.searchQuery}". Try a different search term.
          </p>
        </div>
      )}
      ...
      <Dialog open={deleteConfirmDialogOpen} onOpenChange={handleDeleteConfirmDialogOpenChange}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delete Bullet with Children?</DialogTitle>
            <DialogDescription>
              This bullet has nested items. Deleting it will also delete all its children. This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleDeleteConfirmWithChildren}>
              Delete
            </Button>
          </DialogFooter>
        </DialogContent>
